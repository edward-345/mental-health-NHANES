...
```{r}
# Cleaning DaysMentHlthBad variable for participants older than 18
clean_DaysMentHlthBad <- na.omit(NHANES$DaysMentHlthBad[NHANES$Age > 18])

# Summary statistics for DaysMentHlthBad
summary(clean_DaysMentHlthBad)

# Visualization for DaysMentHlthBad
hist(clean_DaysMentHlthBad, main = "Histogram of Days Mental Health Was Not Good", xlab = "Days", col = "grey")
```
...
```{r}
# log transformation
# NHANES$DaysMentHlthBad <- log(NHANES$DaysMentHlthBad)
# NHANES$DaysMentHlthBad[is.infinite(NHANES$DaysMentHlthBad)] <- 0
# NHANES <- NHANES[NHANES$DaysMentHlthBad != 0, ]

# Fit a multiple regression model using only complete cases for participants older than 18
model_data <- na.omit(NHANES[NHANES$Age > 18, c("DaysMentHlthBad", "BMI", "PhysActiveDays", "HHIncome", "AlcoholYear", "AgeRegMarij", "SleepHrsNight")])
model <- lm(DaysMentHlthBad ~ BMI + PhysActiveDays + HHIncome + AlcoholYear + AgeRegMarij + SleepHrsNight, data = model_data)
...
# Plot the 4 diagnostic plots for the linear model
plot(model)
```
...
```{r}
plot(x = fitted_values, y = model_data$DaysMentHlthBad, main="DaysMentHlthBad vs Fitted",
xlab="Fitted", ylab="DaysMentHlthBad")
...
# Hypothesis tests (t-tests) for coefficients
summary(model)
```

```{r}
# Full model with all predictors
full_model <- lm(DaysMentHlthBad ~ BMI + PhysActiveDays + HHIncome + AlcoholYear + AgeRegMarij + SleepHrsNight, data = model_data)

# Reduced models: Exclude one predictor at a time
model_no_BMI <- lm(DaysMentHlthBad ~ PhysActiveDays + HHIncome + AlcoholYear + AgeRegMarij + SleepHrsNight, data = model_data)
model_no_PhysActiveDays <- lm(DaysMentHlthBad ~ BMI + HHIncome + AlcoholYear + AgeRegMarij + SleepHrsNight, data = model_data)
model_no_HHIncome <- lm(DaysMentHlthBad ~ BMI + PhysActiveDays + AlcoholYear + AgeRegMarij + SleepHrsNight, data = model_data)
model_no_AlcoholYear <- lm(DaysMentHlthBad ~ BMI + PhysActiveDays + HHIncome + AgeRegMarij + SleepHrsNight, data = model_data)
model_no_AgeRegMarij <- lm(DaysMentHlthBad ~ BMI + PhysActiveDays + HHIncome + AlcoholYear + SleepHrsNight, data = model_data)
model_no_SleepHrsNight <- lm(DaysMentHlthBad ~ BMI + PhysActiveDays + HHIncome + AlcoholYear + AgeRegMarij, data = model_data)

# Perform F-tests
anova(model_no_BMI, full_model)
anova(model_no_PhysActiveDays, full_model)
anova(model_no_HHIncome, full_model)
anova(model_no_AlcoholYear, full_model)
anova(model_no_AgeRegMarij, full_model)
anova(model_no_SleepHrsNight, full_model)
```

```{r}
# Final model with significant predictors
final_model <- lm(DaysMentHlthBad ~ HHIncome + SleepHrsNight, data = model_data)
summary(final_model)
```
...
```{r}
# Compare AIC/BIC for models
AIC(full_model, final_model)
BIC(full_model, final_model)
```
...
# Arrange all residual plots (excluding HHIncome) in a grid
grid.arrange(res_sleep_plot, ncol = 2)
```
```{r}
# Residuals vs HHIncome (Boxplot)
res_income_box_plot <- ggplot(model_data, aes(x = as.factor(HHIncome), y = residuals)) +
